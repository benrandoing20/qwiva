...
| **Generate** | `claude-sonnet-4-6` via LiteLLM | streamed, grounded prompt |
| **Cache** | In-process semantic cache | cosine threshold 0.92, 24h TTL |
...
| `NVIDIA_API_KEY` | ✓ | Single key used for embeddings, LLM, and reranker |
...
**Single API key** — all LLM calls (embeddings, reranker, generation) route through the NVIDIA inference hub using one `NVIDIA_API_KEY`.
...
**Agentic routing** — a classifier call (`max_tokens=5`) routes each message to either the full RAG pipeline or a direct response from conversation context. Clinical app defaults to RAG on classifier failure.
...
**Semantic cache** — repeated or near-identical queries (cosine ≥ 0.92) return cached answers within 24 hours, bypassing the retrieval and generation pipeline entirely.
